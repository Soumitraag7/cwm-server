...
import { compareAnswer, comparePassword, hashPassword } from "../utils/authUtil.js";
...
export const forgotPasswordController = async (req, res) => {
  try {
    ...
    // check
    // const user = await userModel.findOne({ email, matchAnswer })
    const user = await userModel.findOne({ email, answer })

    // const matchAnswer = await compareAnswer(answer, user.answer)

    if (!user) { return res.status(404).send({ success: false, message: 'Wrong email or password' }) }
